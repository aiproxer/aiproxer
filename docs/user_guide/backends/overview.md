...
`GET /v1/models` exposes instance-pinned routes such as `cursor-cli-acp.default:cursor/glm-5.2-max`; use one when you need to fix the backend instance. Generic explicit selectors such as `cursor-cli-acp:cursor/grok-4.6-high` can route to any eligible configured Cursor instance. Pinned routes include the full backend instance name, so multiple project-bound instances remain distinct when you use those selectors. The catalog is sourced from the installed Cursor CLI's `agent --list-models` output; when `cursor_api_endpoint` is configured, discovery uses the same `-e <endpoint>` option as the ACP subprocess. Cursor-native CLI IDs such as `cursor-grok-4.5-high` are exposed as `cursor/grok-4.5-high` while the exact CLI ID is retained for process startup.

For Cursor model catalogs that expose paired speed variants, pass the full proxy route in the API `model` field:

| Speed tier | API model slug |
|------------|----------------|
| Grok 4.6 High, standard | `cursor-cli-acp:cursor/grok-4.6-high` |
| Grok 4.6 High, fast | `cursor-cli-acp:cursor/grok-4.6-high-fast` |
| Composer 2.5, standard | `cursor-cli-acp:cursor/composer-2.5` |
| Composer 2.5, fast | `cursor-cli-acp:cursor/composer-2.5-fast` |

To pin a configured backend instance, replace the backend prefix with the instance name, for example `cursor-cli-acp.default:cursor/grok-4.6-high-fast`. Do not append `[fast=false]` or another speed flag to the API slug. The connector derives the requested tier from the `-fast` suffix, selects `fast=false` or `fast=true` through ACP before sending the first prompt, and verifies Cursor's effective value. For known speed-tier routes, the turn fails closed when ACP cannot confirm the requested value.
...
Workspace selection is request-driven, matching other ACP connectors. Each distinct absolute workspace (`session.state.project_dir` or request `project_dir` / `workspace_path` / `cwd` / `project`) gets its own pooled ACP child spawned in that directory; changing folder or git worktree does not require restarting the proxy. Optional `workspace_path` or `CURSOR_CLI_WORKSPACE` is only a default when the request/session does not name a directory; otherwise the process working directory is used as a last resort. Relative paths and prompt-derived paths are not accepted by the connector itself (OpenCode-style `Working directory:` lines are resolved earlier into `session.state.project_dir`). `trust_workspace` / `--trust` only skips Cursor's interactive trust prompt for the chosen directory.
